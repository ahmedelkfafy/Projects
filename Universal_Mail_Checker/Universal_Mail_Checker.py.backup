...
class MailCheckerWorker(QObject):
    def __init__(self, settings):
        ...
        self.stats = {'hits': 0, 'invalids': 0, 'errors': 0, 'checked': 0}
    ...
    def process_result(self, result, live_file, banned_file, unknown_file):
        if result is None:
            return
        ...
        if result['status'] == 'hit':
            ...
            output_line = f"{result['combo']} | {protocol} | {capture}\n"
            live_file.write(output_line)
            live_file.flush()
            ...
            banned_file.write(result['combo'] + '\n')
                            ...
                            for future in done_futures:
                                try:
                                    ...
                                    if result is not None:
                                        self.process_result(result, live_file, banned_file, unknown_file)
                                except Exception as e:
                                    if self.is_running:
